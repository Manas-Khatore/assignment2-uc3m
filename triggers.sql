...
BEGIN
  ...
  FOR rec IN (SELECT * FROM Orders_Clients WHERE username = OLD.username) LOOP
    ...
    VALUES (rec.orderdate, NULL, NULL, rec.dliv_datetime, rec.name, rec.surn1, rec.surn2, rec.bill_waytype, rec.bill_wayname, rec.bill_gate, rec.bill_block, rec.bill_stairw, rec.bill_floor, rec.bill_door, rec.bill_ZIP, rec.bill_town, rec.bill_country, rec.dliv_waytype, rec.dliv_wayname, rec.dliv_gate, rec.dliv_block, rec.dliv_stairw, rec.dliv_floor, rec.dliv_door, rec.dliv_ZIP, rec.dliv_town, rec.dliv_country);
